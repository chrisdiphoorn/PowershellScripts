...
Function GetDiskDriveFree() {  [CmdletBinding()] Param ([Parameter(Mandatory)] [ValidateSet('C:','D:','E:','F:','G:','H:')][string]$DriveLetter)
...
Function GetRightPathName() { Param ([string]$FullPath)
...
Function AddMissingBackSlash() { Param ([string]$FullPath)
...
# ***************************************************************************************************************************************************************************
	...
	$SyncFolders 		= @('E:\FS01\Kiosk','E:\FS01\Servers','E:\FS01\Signatures','E:\FS01\Workstation', 'E:\FS01\Deployment')
    
	$BackupFolders 	= @('D:\BACKUPS\JOII-UW-HV02-Hyper-V\WindowsImageBackup','E:\Applicad')	# Source backup folders to archive.
	$ZipFileNames		= @('JOII-UW-HV02','Applicad')	        # File Names to use when archiving the backup folders.

	$BackupFiles 		= @('\\JOII-UW-SQL01\D$\MSSQL\BACKUP\Payroll_Full*.bak', '\\JOII-UW-SQL01\D$\MSSQL\BACKUP\Joiitech_Ful*.bak', '\\JOII-UW-SQL01\D$\MSSQL\BACKUP\finPOWERConnect_Joii_PROD_Full*.bak')
  
	$DestinationDrive = "F:"
	$ComputerName 	= "JOII-UW-HV01"	        # The Server name running the script. (Used in an Invoke-Command process)

	$DestinationFolder = "$($DestinationDrive)\"  # The folder name on the destination drive to save the backups into.

	$LogFolder 		= "C:\Scripts"		        # The folder where the log files are stored.
	...
	if (Test-Path -Path $DestinationFolder) {
        ...
        foreach($BackupFolder in $BackupFolders)
        {
			...
			if (Test-Path -Path "$($BackupFolder)") { 
				...
				} else { # Test-Path $Destination Missing
          ...
          } else {  # Test-Path $BackupFolder Missing
            if (-NOT($test -eq $True)) { 
